...
for i in "${modulesToBuild[@]}"; do
    ...
    for filename in markdown/*.md; do
        ...
        $PWD:/build ntwrkguru/pandoc-gitlab-ci pandoc -s --mathjax -i -t revealjs "markdown/$name.md" -o "$name.html"
        echo "Building $name.pdf"
        $PWD:/build ntwrkguru/pandoc-gitlab-ci pandoc "markdown/$name.md" -o "$name.pdf" --pdf-engine=pdflatex
    done
