...
if [ $# -eq 0 ]
    then
        for filename in markdown/*.md; do
            file=${filename##*/}
            name=${file%%.*}
            jpbernius/pandoc -s --mathjax -i -t revealjs "markdown/$name.md" -o "$name.html"
            jpbernius/pandoc "markdown/$name.md" -o "$name.pdf" --pdf-engine=pdflatex
        done
else
    filename=$(find ./markdown -name "$1*")
    file=${filename##*/}
    name=${file%%.*}
    jpbernius/pandoc -s --mathjax -i -t revealjs "markdown/$name.md" -o "$name.html" -V revealjs-url=../reveal.js
    jpbernius/pandoc "markdown/$name.md" -o "$name.pdf" --pdf-engine=pdflatex
fi
